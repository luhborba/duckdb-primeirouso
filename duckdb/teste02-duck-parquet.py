...
con.execute(
    "COPY (SELECT * FROM dados) TO 'data/parquet/dados-duck.parquet' (FORMAT PARQUET)"
)

# Salvando total_vendas_produtos-duck.parquet
con.execute(
    "COPY (SELECT Produto, SUM(ValorTotal) AS TotalVendas FROM dados GROUP BY Produto ORDER BY TotalVendas DESC) TO 'data/parquet/total_vendas_produtos-duck.parquet' (FORMAT PARQUET)"
)

# Salvando total_vendas_cliente-duck.parquet
con.execute(
    "COPY (SELECT Cliente, SUM(ValorTotal) AS TotalVendas FROM dados GROUP BY Cliente ORDER BY TotalVendas DESC) TO 'data/parquet/total_vendas_cliente-duck.parquet' (FORMAT PARQUET)"
)
