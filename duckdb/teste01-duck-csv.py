...
#total_vendas_produtos = con.execute("""
#SELECT Produto, SUM(ValorTotal) AS TotalVendas
#FROM dados
#GROUP BY Produto
...
#total_vendas_cliente = con.execute("""
##SELECT Cliente, SUM(ValorTotal) AS TotalVendas
#FROM dados
#GROUP BY Cliente
#ORDER BY TotalVendas DESC
#""").fetchall()
...
con.execute("COPY (SELECT Produto, SUM(ValorTotal) AS TotalVendas FROM dados GROUP BY Produto ORDER BY TotalVendas DESC) TO 'data/csv/dados-duck.csv'")

# Salvando total_vendas_cliente-duck.csv
con.execute("COPY (SELECT Cliente, SUM(ValorTotal) AS TotalVendas FROM dados GROUP BY Cliente ORDER BY TotalVendas DESC) TO 'data/csv/dados-duck.csv'")
